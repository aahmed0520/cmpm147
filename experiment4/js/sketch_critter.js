...
function drawTerrainTile(i, j) {
  ...
  if (trail.some(t => t.i === i && t.j === j)) fill(255, 250, 240, 60), ellipse(0, 0, 12, 12);
  if (flowers.some(f => f.i === i && f.j === j)) {
    fill('pink'); ellipse(-3, -3, 6, 6); ellipse(3, -3, 6, 6); ellipse(-3, 3, 6, 6); ellipse(3, 3, 6, 6);
    fill('yellow'); ellipse(0, 0, 5, 5);
  }
  if (mushrooms.some(m => m.i === i && m.j === j)) fill('brown'), ellipse(0, -2, 8, 6), fill('tan'), rect(-1, -2, 2, 6);
  if (obstacles.some(o => o.i === i && o.j === j)) {
    fill('#444');
    beginShape(); vertex(-tw, 0); vertex(0, -th - 16); vertex(tw, 0); vertex(0, th); endShape(CLOSE);
    fill('#222'); beginShape(); vertex(-tw, 0); vertex(0, th); vertex(0, th + 10); vertex(-tw, 10); endShape(CLOSE);
    beginShape(); vertex(tw, 0); vertex(0, th); vertex(0, th + 10); vertex(tw, 10); endShape(CLOSE);
  }
  if (apples.some(a => a.i === i && a.j === j)) fill('red'), ellipse(0, -3, 16, 16), fill('green'), rect(-1, -12, 2, 5, 1);
