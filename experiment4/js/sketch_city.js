new p5(() => {
      ...
      for (let i = -100; i <= 100; i++) {
        for (let j = -100; j <= 100; j++) {
          if (!isRoadTile(i, j) && !isSidewalkTile(i, j)) {
            if (XXH.h32(`tree:${i},${j}`, worldSeed) % 70 === 0) {
              treeMap.add(`${i},${j}`);
            }
          } else if (isSidewalkTile(i, j) && !isRoadTile(i, j)) {
            if (XXH.h32(`lamp:${i},${j}`, worldSeed) % 30 === 0) {
              lampMap.add(`${i},${j}`);
            }
          }
        }
    ...
    window.p3_drawTile = (i, j) => {
      ...
      if (isRoad) fill(90);
      else if (isSidewalk) fill(160);
      else fill(lerpColor(color('#b8e994'), color('#38ada9'), noise(i * 0.1, j * 0.1)));
